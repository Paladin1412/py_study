...
//这里LinkList 表示为 结构体LNode类型的一个变量。 这里是把结构体strcut LNode 叫作LinkList 
...
{
    int i = 0;
    LinkList p = L->next;/* L是头结点而不是结点指针，结点就具有指针域和数据域，p指向第一个结点即首元结点 */
    while(p)
    {
        i++;
        p = p->next;
...
LinkList ListReverse4(LinkList L)
{
    LinkList p=L;
    if (L == NULL || L->next == NULL)
    {
 ...
 */
void ListReverse5(LinkList p, LinkList L)
{
    if(p->next == NULL)
    {
        L->next = p;//找到最后一个节点
